...
<div class="row justify-content-center">
    <div class="col-lg-8 text-center py-4 ">
        ...
        <form action="/Df/update" method="post"  enctype="multipart/form-data"  >
        @csrf  @method('put')

        <input type="hidden" value = "{{$df->quiz_id}}" name = "quiz_id">
        <input type="hidden" value = "{{$df->id}}" name = "id">

        <div class="form-row">
        <div class="col-md-5 col-sm-12 col-lg-5 ">
            <input type="text" name="question_start" 
            class="form-control @error('question_start') is-invalid @enderror " id="" 
            placeholder="{{__('quiz.question_start_place')}}" value="{{$df->question_start}}" >
        ...
        <div class="col-md-2 col-sm-12 col-lg-2 ">
            <select name="gap" dir = "ltr" class="custom-select"  >
                ...
                <option value = "1" {{ $df->gap == 1 ? 'selected' : '' }}> {{__('quiz.correct_1')}}</option>
                <option value = "2" {{ $df->gap == 2 ? 'selected' : '' }}> {{__('quiz.correct_2')}}</option>
                <option value = "3" {{ $df->gap == 3 ? 'selected' : '' }}> {{__('quiz.correct_3')}}</option>
                <option value = "4" {{ $df->gap == 4 ? 'selected' : '' }}> {{__('quiz.correct_4')}}</option>
            </select>
        </div><!-- END OF FORM COL -->
        ...
        <div class="col-md-5 col-sm-12 col-lg-5 ">
            ...
            placeholder="{{__('quiz.question_end_place')}}" value="{{$df->question_end}}" >
        ...
        <div class="col-md-4 col-sm-12 col-lg-4">
            ...
            <select name="time" dir = "ltr" class="custom-select"  >
                <option value="{{$df->time}}" selected>{{$df->time}} sec</option>
        ...
        <a href="/Quiz/show/{{$df->quiz_id}}" class="btn btn-outline-dark rounded-0 float-left" >برگشت</a>
        </form>
    </div>
</div>
